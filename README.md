...
**이미지 최적화**
- **성능 개선** : Next.js Image 태그를 사용해서 이미지를 최적화하여 페이지 로드 시간을 **200ms** 단축
- **Lighthouse 점수 향상** : 효과적인 이미지 최적화를 통해 Lighthouse 성능 점수가 89에서 **98**로 향상
- **구현 효율성** : Image 태그를 프로젝트 전반에 통합하여 사용자 경험 개선

**Route Handler를 활용한 API 코드 관리**
- **구조 문제** : 프로젝트 초기, 일관되지 않은 API 구조와 중복된 로직으로 유지보수성 및 확장성 저하
- **API 로직 중앙화** : Next.js app/api 라우트 핸들러를 사용해 중앙화된 API 로직 구축
- **구조 개선** : 페이지별 api 요청 로직을 모아서 관리하여 재활용성과 확장성이 용이한 구조로 개편

**폴더구조 개선**
- **고민 지점** : 개인 프로젝트이기에 따라야 할 명확한 폴더구조 기준이 없다고 판단, 효율적인 폴더구조 개선 방안이 무엇일지 고민
- **기준 확립** : 파일을 빠르게 찾을 수 있는지, 해당 파일이 어떤 기능을 수행하는지 즉시 파악할 수 있도록 명확한 구조와 네이밍을 고려
- **구조 개선** : 논리적 그룹화 및 직관적인 네이밍에 중점을 두고 프로젝트 구조를 개편하여 유지 관리성과 효율성 향상
